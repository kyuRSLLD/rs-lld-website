...
const AddProductForm = ({ categories, onAdd, onClose, t }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start sm:items-center justify-center z-50 p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto my-auto">
    ...
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-4 sm:p-5 mb-6">
        <div className="flex items-start gap-3 sm:gap-4">
          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-stone-100 flex items-center justify-center flex-shrink-0">
            <UserCircle className="w-7 h-7 sm:w-8 sm:h-8 text-stone-400" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-stone-900 text-base sm:text-lg truncate">{staff?.full_name || staff?.username}</p>
            <p className="text-sm text-stone-500">@{staff?.username}</p>
            {staff?.email && <p className="text-xs sm:text-sm text-stone-400 truncate">{staff.email}</p>}
          </div>
          <div className="flex-shrink-0">
            <span className="text-xs font-semibold px-2 py-1 rounded-full bg-stone-100 text-stone-600 uppercase tracking-wide">
            ...
            <div className="space-y-2 mb-5">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowCreateOrder(true)}
                  className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white text-xs font-medium rounded-lg hover:bg-blue-700 transition-colors whitespace-nowrap flex-shrink-0">
                  <Plus className="w-3.5 h-3.5" /> <span className="hidden sm:inline">{t.createOrder?.buttonLabel || 'Create Order'}</span><span className="sm:hidden">New</span>
                </button>
                <Button size="sm" variant="outline" onClick={fetchOrders} className="text-xs flex-shrink-0">
                  ...
                  {['', 'pending', 'confirmed', 'packed', 'shipped', 'delivered', 'cancelled'].map(s => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-all whitespace-nowrap flex-shrink-0 ${
                          statusFilter === s
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[
                ...
                <div key={kpi.label} className="bg-white rounded-xl border border-stone-100 p-3 sm:p-4">
                  <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center ${kpi.bg} mb-2 sm:mb-3`}>
                    <kpi.icon className={`w-4 h-4 sm:w-5 sm:h-5 ${kpi.color}`} />
                  </div>
                  <p className="text-xl sm:text-2xl font-bold text-stone-900 truncate">{kpi.value}</p>
                  <p className="text-xs text-stone-500 mt-0.5 leading-tight">{kpi.label}</p>
                </div>
              ))}
            </div>
