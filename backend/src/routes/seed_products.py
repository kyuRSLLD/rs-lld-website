"""
One-time admin endpoint to seed/update the 25 RS LLD featured products.
Protected by SEED_SECRET env var.
POST /api/admin/seed-products  { "secret": "<SEED_SECRET>" }
"""
...
@seed_bp.route('/admin/seed-products', methods=['POST'])
def seed_products():
    secret = request.json.get('secret', '') if request.is_json else ''
    expected = os.environ.get('SEED_SECRET', 'rslld-seed-2024')
    if secret != expected:
